...
**Estado actual:** ver `docs/state/2026-05.md` — **lectura obligatoria** antes de proponer cambios
al motor o al manifesto. Incluye versión activa (V14 v3 con bins temporales calibrados),
sesiones previas, SHADOW pendientes, beads abiertos, y validación rápida del SHA-256.

Cada cambio significativo se appendea como nueva sesión al principio de ese archivo.
Cuando crezca (>500 líneas o cambio de mes), rotar a `docs/state/<año>-<mes>.md` nuevo.
...
Instrumentacion de tokens via skill user-scope `token-efficiency`.
Matriz de decision spawn + patrones: `~/.claude/skills/token-efficiency/references/`.
Log local: `data/logs/agent_tokens.jsonl`. Audit: skill `analyze_tokens` o pregunta directa.
