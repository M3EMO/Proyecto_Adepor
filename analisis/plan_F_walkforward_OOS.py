"""Plan F con walk-forward OOS estricto.

Para cada year_test:
  1. Refit fit_v5_xg + fit_v_ruido + lambda predictors con eventos < year_test.
  2. Construir EMAs forward sobre TODOS los eventos.
  3. Predict + ensemble subsets sobre eventos year=year_test.
  4. Yield + hitrate + Brier por subset y configuracion.

Reportar: yield por (subset, pick_src, div_thr, year). IS pooled walk-forward.
"""
import sqlite3
import math
...
def construir_state_dual_v5(eventos, fit_xg_v5, alfa):
    ...
    for key in keys_ord:
        ...
        xl = calc_xg(ev_l, fit_xg_v5); xv = calc_xg(ev_v, fit_xg_v5)
...
def construir_state_ruido(eventos, fit_xg_ruido, alfa):
    matches = defaultdict(list)
    for ev in eventos:
        key = (ev["liga"], ev["fecha"], ev["equipo"] if ev["es_local"] else ev["rival"],
               ev["rival"] if ev["es_local"] else ev["equipo"])
    ...
    for key in keys_ord:
        ...
        xl = calc_xg(ev_l, fit_xg_ruido); xv = calc_xg(ev_v, fit_xg_ruido)
...
def fit_lambdas_dual_y_ruido(state_dual, state_ruido, year_max):
...
def main():
    ...
    # IS pooled walk-forward por subset/pick/div
    def new_stats():
        return {"n": 0, "stake": 0, "pnl": 0, "hits": 0,
                "by_year": defaultdict(lambda: {"n": 0, "stake": 0, "pnl": 0, "hits": 0})}
    pooled = defaultdict(new_stats)
    ...
    for yt in YEARS_TEST:
        ...
        fit_lam_d, fit_lam_r = fit_lambdas_dual_y_ruido(state_dual, state_ruido, yt)
        if not fit_lam_d: continue
        # Compute lambdas per partido
        ...
        # Compute predicciones para eventos year=yt
        for key, val in emas_v0.items():
            ...
            # Subset
            if pick0 == pickd == pickr: subset = "F0_3agree"
            elif pick0 == pickr and pickd != pick0: subset = "F1_v0=ruido!=dual"
            elif pick0 == pickd and pickr != pick0: subset = "F2_v0=dual!=ruido"
            elif pickd == pickr and pick0 != pickd: subset = "F3_dual=ruido!=v0"
            ...
            for pick_src, (pl, pe, pv, pick) in [("V0",(pl0,pe0,pv0,pick0)),
                ...
                ev = p_pick * cuota_pick
                if ev < EV_MIN: continue
                for thr in DIV_THRS:
                    if divergencia < thr: continue
                    skey = (subset, pick_src, thr)
                    s = pooled[skey]
                    s["n"] += 1; s["stake"] += 1; s["hits"] += int(pick == out)
                    s["pnl"] += (cuota_pick - 1.0) if pick == out else -1.0
                    sb = s["by_year"][yt]
                    sb["n"] += 1; sb["stake"] += 1; sb["hits"] += int(pick == out)
                    sb["pnl"] += (cuota_pick - 1.0) if pick == out else -1.0

    # Reportar IS pooled
    print("\n" + "="*120)
    print("PLAN F WALK-FORWARD OOS — IS pooled (suma de years 2023-2026)")
    print("="*120)
    print(f"{'subset':<22s}{'pick':<8s}{'div':<6s}{'N_IS':>8s}{'hit%':>8s}{'yield_IS%':>11s}")
    for skey in sorted(pooled.keys()):
        subset, pick_src, thr = skey
        s = pooled[skey]
        if s["n"] == 0 or s["n"] < 5: continue
        hit = s["hits"]/s["n"]*100
        yld = s["pnl"]/s["stake"]*100 if s["stake"] else 0
        print(f"{subset:<22s}{pick_src:<8s}{thr:<6.2f}{s['n']:>8d}{hit:>7.2f}%{yld:>10.2f}%")

    # Reportar por anio (focus subsets prometedores)
    print("\n" + "="*120)
    print("PLAN F WALK-FORWARD — yield por anio (subsets F2 + F3 + F0 con N>=5)")
    print("="*120)
    print(f"{'subset':<22s}{'pick':<8s}{'div':<6s}{'2023':>10s}{'2024':>10s}{'2025':>10s}{'2026':>10s}{'IS':>10s}")
    for skey in sorted(pooled.keys()):
        subset, pick_src, thr = skey
        if subset not in ("F0_3agree", "F2_v0=dual!=ruido", "F3_dual=ruido!=v0"): continue
        s = pooled[skey]
        if s["n"] < 10: continue
        row = f"{subset:<22s}{pick_src:<8s}{thr:<6.2f}"
        for yt in YEARS_TEST:
        ...
        is_yld = s["pnl"]/s["stake"]*100 if s["stake"] else 0
        row += f"{is_yld:>9.2f}%"
        print(row)
