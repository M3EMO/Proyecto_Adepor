"""
SOFA patterns RAPIDOS — F1 + F2 + E sequencing lag-N.

F1: xG over/underperformance EMA (goals - xg_shotmap, span=5) por equipo
    -> Hipotesis regresion a la media: equipo lucky reverte, unlucky reverte.
F2: Big-chance conversion rate EMA por equipo
    -> Equipos con BCC bajo tienen "deuda" -> reversion proximo
E:  Sequencing lag-N: para cada feature SOFA, tomar lag-1 + lag-3 (mean) + lag-5 (mean)
    -> Test si lag-N suaviza ruido lag-1

Pipeline:
1. Construir snapshot per (liga, equipo, fecha) con EMA shotmap-derived
2. Cruzar con universo cuotas (SOFA 2026 ∩ partidos_backtest/fdco)
3. Bin q4 + bootstrap CI95 + Bonferroni
4. Reporte y SHADOW persist
"""
from __future__ import annotations
import sqlite3
import json
import math
import re
import unicodedata
import numpy as np
from pathlib import Path
from collections import defaultdict
from datetime import datetime

DB = "fondo_quant.db"
ROOT = Path(__file__).resolve().parents[1]
np.random.seed(42)


def norm(s):
    if not s: return ""
    s = unicodedata.normalize("NFKD", s).encode("ascii", "ignore").decode().lower()
    s = re.sub(r"[^a-z0-9]", "", s)
    return s


def bootstrap_ci(values, n_boot=1000, alpha=0.05):
    if len(values) < 2: return (float("nan"), float("nan"))
    arr = np.asarray(values, dtype=float)
    samples = np.random.choice(arr, size=(n_boot, len(arr)), replace=True)
    means = np.sort(samples.mean(axis=1))
    return (float(means[int(alpha/2*n_boot)]), float(means[int((1-alpha/2)*n_boot)]))


def main() -> None:
    con = sqlite3.connect(DB)
    cur = con.cursor()

    # 1. Cargar SOFA con todos los datos relevantes
    sofa = list(cur.execute("""
        SELECT sofa_event_id, liga, fecha, ht, at, hg, ag,
               xg_shotmap_l, xg_shotmap_v,
               big_chances_l, big_chances_v,
               big_chances_missed_l, big_chances_missed_v,
               shots_total_l, shots_total_v, shots_on_target_l, shots_on_target_v,
               touches_penalty_area_l, touches_penalty_area_v,
               errors_lead_to_shot_l, errors_lead_to_shot_v
        FROM sofascore_match_features
        WHERE error IS NULL
              AND hg IS NOT NULL AND ag IS NOT NULL
              AND xg_shotmap_l IS NOT NULL AND xg_shotmap_v IS NOT NULL
        ORDER BY liga, fecha
    """))
    print(f"SOFA con xG y goles: {len(sofa)}")

    # 2. Construir histories por (liga, equipo) con stats POST-match para usar como lag features
    # Cada entry: (fecha, role 'l'|'v', stats_dict)
    histories = defaultdict(list)
    for row in sofa:
        sid, liga, fecha, ht, at, hg, ag, xgl, xgv, bcl, bcv, bcml, bcmv, sl, sv, sotl, sotv, tpal, tpav, errl, errv = row
        # local view
        if bcl is not None and bcml is not None:
            bc_anot_l = max(0, bcl - bcml)
            bcc_l = (bc_anot_l / bcl) if bcl > 0 else None
        else:
            bcc_l = None
        histories[(norm(liga), norm(ht))].append((fecha[:10], "l", {
            "xg_perf": hg - xgl,  # goals - xg
            "bcc": bcc_l,
            "bc_total": bcl,
            "shots": sl, "sots": sotl, "tpa": tpal, "errors": errl,
        }))
        if bcv is not None and bcmv is not None:
            bc_anot_v = max(0, bcv - bcmv)
            bcc_v = (bc_anot_v / bcv) if bcv > 0 else None
        else:
            bcc_v = None
        histories[(norm(liga), norm(at))].append((fecha[:10], "v", {
            "xg_perf": ag - xgv,
            "bcc": bcc_v,
            "bc_total": bcv,
            "shots": sv, "sots": sotv, "tpa": tpav, "errors": errv,
        }))

    # Sort each history by fecha
    for k in histories:
        histories[k].sort(key=lambda x: x[0])

    def get_lag_features(liga, equipo, fecha, n_lag=5):
        """Devuelve EMA o mean de stats sobre los last n_lag partidos PREVIOS del equipo."""
        hist = histories.get((norm(liga), norm(equipo)), [])
        prev = [h for h in hist if h[0] < fecha]
        if len(prev) < 3:  # warmup
            return None
        last_n = prev[-n_lag:] if n_lag else prev
        # Compute mean (EMA approx) of each stat
        stats_collected = defaultdict(list)
        for f, role, stats in last_n:
            for k, v in stats.items():
                if v is not None:
                    stats_collected[k].append(v)
        result = {}
        # Goals - xG cumulative (for over/underperformance)
        result["xg_perf_ema"] = sum(stats_collected["xg_perf"]) / len(stats_collected["xg_perf"]) if stats_collected["xg_perf"] else None
        result["bcc_ema"] = sum(stats_collected["bcc"]) / len(stats_collected["bcc"]) if stats_collected["bcc"] else None
        result["bc_total_ema"] = sum(stats_collected["bc_total"]) / len(stats_collected["bc_total"]) if stats_collected["bc_total"] else None
        result["shots_ema"] = sum(stats_collected["shots"]) / len(stats_collected["shots"]) if stats_collected["shots"] else None
        result["sots_ema"] = sum(stats_collected["sots"]) / len(stats_collected["sots"]) if stats_collected["sots"] else None
        result["tpa_ema"] = sum(stats_collected["tpa"]) / len(stats_collected["tpa"]) if stats_collected["tpa"] else None
        result["errors_ema"] = sum(stats_collected["errors"]) / len(stats_collected["errors"]) if stats_collected["errors"] else None
        result["n_lag"] = len(last_n)
        return result

    # 3. Index cuotas
    cuotas_idx = {}
    for r in cur.execute("""
        SELECT pais, fecha, local, visita, cuota_1, cuota_x, cuota_2, cuota_o25, cuota_u25
        FROM partidos_backtest WHERE estado='Liquidado' AND fecha>='2026' AND cuota_1 IS NOT NULL
    """):
        liga, fecha, ht, at, c1, cx, c2, co25, cu25 = r
        k = (norm(liga), fecha[:10], norm(ht), norm(at))
        cuotas_idx[k] = (c1, cx, c2, co25, cu25, "backtest")
    for r in cur.execute("""
        SELECT liga, fecha, equipo_local, equipo_visita,
               cuota_1, cuota_x, cuota_2, cuota_o25, cuota_u25
        FROM cuotas_historicas_fdco WHERE fecha>='2026' AND cuota_1 IS NOT NULL
    """):
        liga, fecha, ht, at, c1, cx, c2, co25, cu25 = r
        k = (norm(liga), fecha[:10], norm(ht), norm(at))
        if k not in cuotas_idx:
            cuotas_idx[k] = (c1, cx, c2, co25, cu25, "fdco")
    cuotas_loose = defaultdict(list)
    for k, v in cuotas_idx.items():
        cuotas_loose[(k[1], k[2][:6])].append((k, v))

    def get_cuotas(liga, fecha, ht, at):
        k = (norm(liga), fecha[:10], norm(ht), norm(at))
        if k in cuotas_idx: return cuotas_idx[k]
        for kk, vv in cuotas_loose.get((k[1], k[2][:6]), []):
            if kk[3][:5] == k[3][:5]: return vv
        return None

    # 4. Construir universo eventos: para cada partido SOFA, lag features local + visita + cuotas
    eventos = []
    n_no_lag = n_no_cuotas = 0
    for row in sofa:
        sid, liga, fecha, ht, at, hg, ag, xgl, xgv, bcl, bcv, *_ = row
        feats_l_lag5 = get_lag_features(liga, ht, fecha, n_lag=5)
        feats_v_lag5 = get_lag_features(liga, at, fecha, n_lag=5)
        feats_l_lag3 = get_lag_features(liga, ht, fecha, n_lag=3)
        feats_v_lag3 = get_lag_features(liga, at, fecha, n_lag=3)
        if not feats_l_lag5 or not feats_v_lag5:
            n_no_lag += 1
            continue

        cuotas = get_cuotas(liga, fecha, ht, at)
        if not cuotas:
            n_no_cuotas += 1
            continue
        c1, cx, c2, co25, cu25, fuente = cuotas

        if hg > ag: res = "1"
        elif hg < ag: res = "2"
        else: res = "X"

        def yld(p, cu):
            if cu is None or cu <= 1.0: return None
            hit = 1 if p == res else 0
            return (cu - 1.0) if hit else -1.0

        e = {
            "sid": sid, "liga": liga, "fecha": fecha[:10], "ht": ht, "at": at,
            "hg": hg, "ag": ag, "res": res, "fuente": fuente,
            "cuota_1": c1, "cuota_x": cx, "cuota_2": c2, "cuota_o25": co25, "cuota_u25": cu25,
            "yield_local": yld("1", c1),
            "yield_empate": yld("X", cx),
            "yield_visita": yld("2", c2),
            "yield_o25": ((co25-1.0) if co25 and co25>1 and (hg+ag>2) else (-1.0 if co25 and co25>1 else None)),
            "yield_u25": ((cu25-1.0) if cu25 and cu25>1 and (hg+ag<=2) else (-1.0 if cu25 and cu25>1 else None)),
            "hit_o25": (1 if hg+ag>2 else 0) if co25 else None,
            "hit_u25": (1 if hg+ag<=2 else 0) if cu25 else None,
        }

        # Lag features con sufijo _l_lag5, _v_lag5, _l_lag3, _v_lag3
        for prefix, feats in [("l_lag5", feats_l_lag5), ("v_lag5", feats_v_lag5),
                              ("l_lag3", feats_l_lag3), ("v_lag3", feats_v_lag3)]:
            if feats is None: continue
            for k, v in feats.items():
                if k == "n_lag": continue
                e[f"{k}_{prefix}"] = v
        # Diff l - v
        for k_base in ["xg_perf_ema", "bcc_ema", "bc_total_ema", "shots_ema", "sots_ema", "tpa_ema", "errors_ema"]:
            for span in ["lag5", "lag3"]:
                vl = e.get(f"{k_base}_l_{span}")
                vv = e.get(f"{k_base}_v_{span}")
                if vl is not None and vv is not None:
                    e[f"diff_{k_base}_{span}"] = vl - vv
                    e[f"asim_l_atk_v_def_{k_base}_{span}"] = vl - vv
                else:
                    e[f"diff_{k_base}_{span}"] = None

        eventos.append(e)

    print(f"\nUniverso eventos: {len(eventos)}")
    print(f"Sin lag: {n_no_lag}")
    print(f"Sin cuotas: {n_no_cuotas}")

    # Baselines
    baselines = {}
    for pick in ["local", "visita", "empate", "o25", "u25"]:
        ys = [e[f"yield_{pick}"] for e in eventos if e.get(f"yield_{pick}") is not None]
        if ys:
            baselines[pick] = sum(ys) / len(ys)
            print(f"  baseline {pick}: {baselines[pick]:+.3%} N={len(ys)}")

    # 5. Lista de features a testear
    features = []
    for k_base in ["xg_perf_ema", "bcc_ema", "bc_total_ema", "shots_ema", "sots_ema", "tpa_ema", "errors_ema"]:
        for span in ["lag5", "lag3"]:
            for side in ["l", "v"]:
                features.append(f"{k_base}_{side}_{span}")
            features.append(f"diff_{k_base}_{span}")

    print(f"\nFeatures a testear: {len(features)}")

    targets = ["yield_local", "yield_visita", "yield_empate", "yield_o25", "yield_u25"]

    findings = []
    n_tests = 0
    for feat in features:
        for target in targets:
            pick = target.replace("yield_", "")
            base = baselines.get(pick, 0)
            vals = [(e.get(feat), e.get(target)) for e in eventos
                    if e.get(feat) is not None and e.get(target) is not None]
            if len(vals) < 50: continue
            arr = np.array([v[0] for v in vals], dtype=float)
            quantiles = np.unique(np.percentile(arr, np.linspace(0, 100, 5)))
            if len(quantiles) < 2: continue
            for i in range(len(quantiles)-1):
                lo, hi = quantiles[i], quantiles[i+1]
                is_last = (i == len(quantiles)-2)
                if is_last:
                    sub = [v for v in vals if lo <= v[0] <= hi]
                else:
                    sub = [v for v in vals if lo <= v[0] < hi]
                if len(sub) < 30: continue
                n_tests += 1
                ys = np.array([v[1] for v in sub])
                ymean = float(ys.mean())
                lift = ymean - base
                ci_lo, ci_hi = bootstrap_ci(ys)
                if lift > 0.05 and ci_lo > 0:
                    findings.append({
                        "feature": feat, "target": target, "pick": pick,
                        "lo": float(lo), "hi": float(hi), "n": len(sub),
                        "yield_mean": ymean, "ci95_lo": ci_lo, "ci95_hi": ci_hi,
                        "baseline": base, "lift": lift,
                    })

    bonf_alpha = 0.05 / max(1, n_tests)
    print(f"\nTests: {n_tests}  Bonferroni alpha: {bonf_alpha:.7f}")
    print(f"Findings (lift>+5pp AND CI95_lo>0): {len(findings)}")

    findings.sort(key=lambda x: x["lift"], reverse=True)
    print()
    print("=== TOP 30 findings ===")
    print(f"{'feature':<40} {'pick':<8} {'N':>4} {'yield':>8} {'lift':>8} {'CI95_lo':>8} {'CI95_hi':>8}")
    for f in findings[:30]:
        print(f"{f['feature'][:39]:<40} {f['pick']:<8} {f['n']:>4} {f['yield_mean']:>+8.3%} {f['lift']:>+8.3%} {f['ci95_lo']:>+8.3f} {f['ci95_hi']:>+8.3f}")

    # 6. SHADOW persistencia para findings con lift > 5pp Y CI95_lo > 0
    cur.execute("DROP TABLE IF EXISTS picks_shadow_sofa_patterns_rapidos_v1")
    cur.execute("""
        CREATE TABLE picks_shadow_sofa_patterns_rapidos_v1 (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ts_log TEXT, sofa_event_id INTEGER,
            liga TEXT, fecha TEXT, ht TEXT, at TEXT,
            fuente_cuota TEXT,
            patron TEXT, filtro_id TEXT, filtro_descripcion TEXT,
            filtro_feature TEXT, filtro_lo REAL, filtro_hi REAL,
            pick TEXT, cuota REAL,
            hit_real INTEGER, yield_real REAL,
            n_acum_filtro INTEGER, yield_acum_filtro REAL,
            ci95_lo_pool REAL, yield_pool_validation REAL, n_pool_validation INTEGER,
            bonferroni_alpha REAL,
            aplicado_produccion INTEGER DEFAULT 0,
            razon_no_aplicado TEXT
        )
    """)
    cur.execute("CREATE INDEX idx_pssprv1_filtro ON picks_shadow_sofa_patterns_rapidos_v1 (filtro_id)")

    ts_log = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    pick_to_cuota = {"local": "cuota_1", "empate": "cuota_x", "visita": "cuota_2",
                     "o25": "cuota_o25", "u25": "cuota_u25"}
    pick_to_short = {"local": "1", "empate": "X", "visita": "2", "o25": "O25", "u25": "U25"}
    pick_to_hit = {"local": lambda e: 1 if e["res"]=="1" else 0,
                   "empate": lambda e: 1 if e["res"]=="X" else 0,
                   "visita": lambda e: 1 if e["res"]=="2" else 0,
                   "o25": lambda e: e.get("hit_o25"), "u25": lambda e: e.get("hit_u25")}

    rows = []
    for f in findings:
        feat = f["feature"]
        pick = f["pick"]
        # Determinar patron
        if "xg_perf" in feat: patron = "F1_xg_overperformance"
        elif "bcc" in feat: patron = "F2_big_chance_conversion"
        else: patron = f"E_sequencing_{feat.split('_')[-1]}"

        n_acum = 0
        yld_sum = 0
        for e in eventos:
            if e.get(feat) is None: continue
            if not (f["lo"] <= e[feat] <= f["hi"]): continue
            cuota = e.get(pick_to_cuota[pick])
            if cuota is None or cuota <= 1.0: continue
            yld = e.get(f"yield_{pick}")
            if yld is None: continue
            hit = pick_to_hit[pick](e)
            n_acum += 1; yld_sum += yld
            yld_acum = yld_sum / n_acum
            filtro_id = f"{patron}|{feat}|q[{f['lo']:.3f},{f['hi']:.3f}]|{pick}"
            rows.append((
                ts_log, e["sid"], e["liga"], e["fecha"], e["ht"], e["at"],
                e["fuente"], patron, filtro_id,
                f"{feat} in [{f['lo']:.3f}, {f['hi']:.3f}] -> {pick}",
                feat, f["lo"], f["hi"],
                pick_to_short[pick], cuota, hit, yld,
                n_acum, yld_acum,
                f["ci95_lo"], f["yield_mean"], f["n"],
                bonf_alpha, 0,
                "shadow_pendiente_n80_y_oos_temporadas_proximas",
            ))

    cur.executemany("""
        INSERT INTO picks_shadow_sofa_patterns_rapidos_v1 (
            ts_log, sofa_event_id, liga, fecha, ht, at, fuente_cuota,
            patron, filtro_id, filtro_descripcion,
            filtro_feature, filtro_lo, filtro_hi,
            pick, cuota, hit_real, yield_real,
            n_acum_filtro, yield_acum_filtro,
            ci95_lo_pool, yield_pool_validation, n_pool_validation,
            bonferroni_alpha, aplicado_produccion, razon_no_aplicado
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, rows)
    con.commit()

    print(f"\nSHADOW persisted: {len(rows)} picks")
    print()
    summary = list(cur.execute("""
        SELECT patron, COUNT(DISTINCT filtro_id), COUNT(*) AS n_picks,
               AVG(yield_real) AS yield_acum, AVG(hit_real) AS hit_acum
        FROM picks_shadow_sofa_patterns_rapidos_v1
        GROUP BY patron
        ORDER BY patron
    """))
    print("Resumen por patron:")
    print(f"{'patron':<35} {'n_filtros':>9} {'n_picks':>8} {'yield_acum':>10} {'hit_acum':>9}")
    for p, nf, n, y, h in summary:
        print(f"{p:<35} {nf:>9} {n:>8} {y:>+10.3%} {h:>9.1%}")

    out = ROOT / "analisis" / "sofa_patterns_rapidos.json"
    out.write_text(json.dumps({
        "ts_log": ts_log,
        "n_eventos": len(eventos),
        "n_features": len(features),
        "n_tests": n_tests,
        "bonferroni_alpha": bonf_alpha,
        "n_findings_lift_5pp_ci_pos": len(findings),
        "findings_top": findings[:50],
        "n_picks_shadow": len(rows),
        "summary_per_patron": [{"patron": p, "n_filtros": nf, "n_picks": n,
                                 "yield_acum": y, "hit_acum": h}
                                for p, nf, n, y, h in summary],
    }, indent=2, default=float, ensure_ascii=False), encoding="utf-8")


if __name__ == "__main__":
    main()
